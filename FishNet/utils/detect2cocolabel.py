...
parser.add_argument('--root_dir', default='../dataset_test', type=str,
                    help="root path of images and labels, include ./images and ./labels and classes.txt")
parser.add_argument('--save_path', type=str, default='./detect_label.json',
                    help="if not split the dataset, give a path to a json file")
parser.add_argument('--random_split', default=False,action='store_true', help="random split the dataset, default ratio is 8:1:1")
parser.add_argument('--split_by_file', default=False,action='store_true',
...
def yolo2coco(arg):
    ...
    originLabelsDir = os.path.join(root_path, 'detect')
    originImagesDir = os.path.join(root_path, 'images')
    with open(os.path.join(root_path, 'detect/classes.txt')) as f:
